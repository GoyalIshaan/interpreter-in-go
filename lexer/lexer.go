...
func (l *Lexer) NextToken() token.Token {
	...
	case '=':
		if l.peekChar() == '=' {
			currentChar := l.ch
			l.readChar()
			tok = token.Token{Type: token.EQ, Literal: string(currentChar) + string(l.ch)}
		} else {
	...
	case '!':
		if l.peekChar() == '=' {
			currentChar := l.ch
			l.readChar()
			tok = token.Token{Type: token.NOT_EQ, Literal: string(currentChar) + string(l.ch)}
		} else {
...
func (l *Lexer) readIdentifier() string {
	origianlPosition := l.position
	for isLetter(l.ch) {
		l.readChar()
	}
	return l.input[origianlPosition:l.position]
}
